        ...
        private fun initPhrasesDb() {  //初始化常用语数据数据
            ...
            if(instance.skbFunDao().getAllMenu().isEmpty()) {
                val skbFuns = listOf(
                    ...
                    SkbFun(name = SkbMenuMode.NumberRow.name, isKeep = 0, position = 7),
                    SkbFun(name = SkbMenuMode.JianFan.name, isKeep = 0, position = 8),
                    SkbFun(name = SkbMenuMode.Mnemonic.name, isKeep = 0, position = 9),
                    // SkbMenuMode.FloatKeyboard removed
                    // SkbMenuMode.FlowerTypeface removed
